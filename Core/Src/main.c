...
ST7789V3_Config st7789_config = {
    .spi_write = spi_write,
    .spi_write_dma = NULL, // Set to NULL to disable DMA
...
static void app_delay_ms(uint32_t milliseconds) {
  ...
  // Uses RTOS Aware delay if scheduler is running, otherwise falls back to HAL_Delay

  //RTOS delay is non blocking more efficient to use with RTOS vs Blocking HAL_Delay
...
int8_t spi_write(uint16_t len, const uint8_t *pData) {
  HAL_StatusTypeDef status =
      HAL_SPI_Transmit(&hspi1, pData, len, ST7789_SPI_TIMEOUT_MS);
  if (status != HAL_OK) {
    return -1;
  }
...
int8_t I2C_Write(uint16_t dev_addr, uint16_t reg_addr, uint8_t *p_data,
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Mem_Write(&hi2c1, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT,
                        p_data, len, MPU6500_I2C_TIMEOUT_MS);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, LCD_DC_Pin|LCD_RES_Pin|LCD_CS_Pin, GPIO_PIN_RESET);
